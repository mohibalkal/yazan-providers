...
export const goMoviesScraper = makeSourcerer({
  ...
  async scrapeShow(ctx) {
    ...
    const mediaData: MediaData[] = mediaElements.toArray().map((movieEl) => {
      const $movieEl = searchPage(movieEl);
      return {
        name: $movieEl.find('h2.film-name a').text(),
        year: $movieEl.find('span.fdi-item:first').text(),
        path: $movieEl.find('h2.film-name a').attr('href'),
      };
    });
    ...
    const seasonsDoc = load(seasons);
    const seasonsEl = seasonsDoc('.ss-item');

    const seasonsData: SeasonData[] = seasonsEl.toArray().map((season) => ({
      number: seasonsDoc(season).text().replace('Season ', ''),
      dataId: seasonsDoc(season).attr('data-id') || '',
    }));
    ...
    const episodesData: EpisodeData[] = episodesEl.toArray().map((ep) => ({
      dataId: episodesPage(ep).attr('data-id') || '',
    ...
    const filteredEmbeds = embeds
      .filter((embed): embed is { embedId: string; url: string } => !!embed.url)
      .map((embed) => ({
