...
export async function getFlixhqShowSources(ctx: ScrapeContext, media: ShowMedia, id: string) {
  ...
  const seasonEl = seasonsDoc('.dropdown-item')
    .toArray()
    .find((el) => seasonsDoc(el).text() === `Season ${media.season.number}`);
  
  const season = seasonEl ? seasonsDoc(seasonEl).attr('data-id') : undefined;
